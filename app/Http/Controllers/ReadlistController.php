...
class ReadlistController extends Controller
{
    ...
    /**
     * Get all readlists for a user
     * 
     * @param \Illuminate\Http\Request $request
     * @return \Illuminate\Http\JsonResponse
     */
    public function getUserReadlists(Request $request)
    {
        $currentUser = auth()->user();
        $targetUserId = $request->query('user_id');
        
        try {
            // If user_id is provided, fetch that user's public readlists
            ...
            if ($targetUserId) {
                ...
                // Build query for target user
                $query = \App\Models\Readlist::where('user_id', $userId)
                    ->where(function($q) {
                        $q->whereNull('is_system')->orWhere('is_system', false);
                    });
                
                // If not the owner, only show public readlists
                if (!$isOwner) {
    ...
    public function show($id)
    {
        try {
            ...
            foreach ($items as $item) {
                ...
                } elseif ($item->item_type === 'external' || $item->item_type === 'link') {
