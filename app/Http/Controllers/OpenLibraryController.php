...
class OpenLibraryController extends Controller
{
    ...
    /**
     * Get separate entries (URLs) added by a user.
     * 
    ...
    public function getUserEntries(Request $request)
    {
        try {
            ...
            // If user_id is provided, fetch that user's entries
            // Otherwise, fetch the authenticated user's entries
            if ($targetUserId) {
    ...
    private function fetchLibraryStructure($id)
    {
        try {
            ...
            $urls = !empty($urlIds) ? LibraryUrl::with(['creator:id,username'])
                ->whereIn('id', $urlIds)
                ->select('id', 'title', 'url', 'summary', 'created_at', 'created_by')
                ->get()->keyBy('id') : collect();
