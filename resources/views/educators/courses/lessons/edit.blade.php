...
<div class="space-y-6">
    ...
    <!-- Form -->
    <div class="card">
        <form action="{{ route('educator.courses.lessons.update', ['courseId' => $course->id, 'lessonId' => $lesson->id]) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="description" name="description" rows="3"
                            class="form-input" placeholder="Enter lesson description (optional)">{{ old('description', $lesson->description) }}</textarea>
                    </div>
                    <div>
                        ...
                        <select id="content_type" name="content_type" required class="form-input" onchange="showContentFields()">
                            ...
                            <option value="quiz" {{ old('content_type', $lesson->content_type) == 'quiz' ? 'selected' : '' }}>Quiz</option>
                            <option value="assignment" {{ old('content_type', $lesson->content_type) == 'assignment' ? 'selected' : '' }}>Assignment</option>
                        </select>
                    </div>
                    <div>
                ...
                <div class="space-y-4">
                    @if($lesson->video_url)
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <div class="flex items-center">
                                <i class="fa-solid fa-video text-primary-500 mr-3 text-xl"></i>
                                <div>
                                    <p class="text-sm font-medium text-gray-700">Current Video</p>
                                    <p class="text-xs text-gray-500">A video is already uploaded. Upload a new one to replace it.</p>
                                </div>
                            </div>
                        </div>
                    @endif
                    <div>
                        <label for="video" class="block text-sm font-medium text-gray-700 mb-1">Video File {{ $lesson->video_url ? '' : '<span class="text-red-600">*</span>' }}</label>
                    ...
                    <div>
                        ...
                        @if($lesson->thumbnail_url)
                            <div class="mb-2">
                                <img src="{{ $lesson->thumbnail_url }}" alt="{{ $lesson->title }}" class="h-32 object-cover rounded-md">
                            </div>
                        @endif
            ...
            <div id="document_fields" class="border-b border-gray-200 pb-6 hidden">
                ...
                @if($lesson->file_url)
                    <div class="bg-gray-50 p-4 rounded-lg mb-4">
                        <div class="flex items-center">
                            <i class="fa-solid fa-file-alt text-primary-500 mr-3 text-xl"></i>
                            <div>
                                <p class="text-sm font-medium text-gray-700">Current Document</p>
                                <p class="text-xs text-gray-500">A document is already uploaded. Upload a new one to replace it.</p>
                            </div>
                        </div>
                    </div>
                @endif
                <div>
                    <label for="file" class="block text-sm font-medium text-gray-700 mb-1">Document File {{ $lesson->file_url ? '' : '<span class="text-red-600">*</span>' }}</label>
            ...
            <div class="flex justify-end space-x-3 pt-3">
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="fa-solid fa-save mr-2"></i> Update Lesson
                </button>
            </div>
        </form>
    </div>
